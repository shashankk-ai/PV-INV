...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Quote every field — handles commas/quotes in item names and formatted numbers
      ...
      const csvRows = [
        ...
        ...rows.map((r) => [
          ...
          q(r.system_qty), q(fmt(r.system_value)), q(fmt(r.avg_cost)),
          q(r.pv_qty), q(fmt(r.pv_value)), q(fmt(r.value_diff)),
          q(r.qty_diff), q(r.status.toUpperCase()),
        ].join(',')),
        '',
